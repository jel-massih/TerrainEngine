...
{
	if(!InitializeShader(device, hwnd, L"font.vs", L"font.ps"))
	{
		return false;
	}
...
{
	if(!SetShaderParamaters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, pixelColor))
	{
		return false;
	}
...
bool FontShaderClass::SetShaderParamaters(ID3D11DeviceContext* deviceContext, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatix, ID3D11ShaderResourceView* texture, XMFLOAT4 pixelColor)
{
	HRESULT result;
