...
{
	if(!InitializeShader(device, hwnd, L"terrain.vs", L"terrain.ps"))
	{
		return false;
	}
...
bool TerrainShaderClass::Render(ID3D11DeviceContext* deviceContext,int indexCount,
							  XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatrix,
							  XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, ID3D11ShaderResourceView* texture)
{
	if(!SetShaderParamaters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightDirection, ambientColor, diffuseColor, texture))
	{
		return false;
	}
...
bool TerrainShaderClass::SetShaderParamaters(ID3D11DeviceContext* deviceContext, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatix,
										   XMFLOAT3 lightDirection, XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
	...
	projectionMatix = XMMatrixTranspose(projectionMatix);

	dataPtr->world = worldMatrix;
	dataPtr->view = viewMatrix;
	dataPtr->projection = projectionMatix;
...
void TerrainShaderClass::RenderShaders(ID3D11DeviceContext* deviceContext, int indexCount)
{
